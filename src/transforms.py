...
class PseudoLabel(nn.Module):
    ...
    def scale_(self, image, target):
        ...
        r_image = F.resize(image, [new_height, new_width], interpolation=self.interpolation)
        if target is not None:
            r_target = copy.deepcopy(target)
            r_target["boxes"][:, 0::2] = target["boxes"][:, 0::2] * new_width / orig_width
            r_target["boxes"][:, 1::2] = target["boxes"][:, 1::2] * new_height / orig_height
            if "masks" in target:
                r_target["masks"] = F.resize(
                    target["masks"], [new_height, new_width], interpolation=torchvision.transforms.InterpolationMode.NEAREST
                )
        else:
            target = None

        r_image = F.pil_to_tensor(r_image)
        r_image = F.convert_image_dtype(r_image, torch.float)
    ...
    def flip_(self, image, target):
        ...
        if target is not None:
            f_target = copy.deepcopy(target)
            width, _ = F.get_image_size(f_image)
            f_target["boxes"][:, [0, 2]] = width - f_target["boxes"][:, [2, 0]]

            is_valid = (f_target["boxes"][:, 0] < f_target["boxes"][:, 2]) & (f_target["boxes"][:, 1] < f_target["boxes"][:, 3])
            f_target["boxes"] = f_target["boxes"][is_valid]
            f_target["labels"] = f_target["labels"][is_valid]
            f_target["scores"] = f_target["scores"][is_valid]

        else:
            f_target = None

        f_image = F.pil_to_tensor(f_image)
    ...
    def forward(self, image, target):
        ...
        data_dict = {"normal": (F.convert_image_dtype(F.pil_to_tensor(image), torch.float), target)}
